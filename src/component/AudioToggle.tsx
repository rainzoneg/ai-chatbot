...
export default function AudioToggle({ isAudioEnabled, toggleAudio }: Props) {
  return (
    <div
      ...
      className="flex flex-row items-center text-left ml-auto gap-2 hover:cursor-pointer"
    >
      {isAudioEnabled ? (
        <>
          <Icon icon="mdi:volume-high" className="w-auto text-white" />
          <p className="font-semibold text-white text-sm">Audio</p>
        </>
      ) : (
        <>
          <Icon icon="mdi:volume-off" className="w-auto text-white" />
          <p className="font-semibold text-gray-400/70 text-sm">Audio</p>
        </>
      )}
    </div>
  );
